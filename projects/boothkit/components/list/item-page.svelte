<script lang="ts">
  import type { ItemPage } from "../../parsers/item";
  ...
  let allMedia = $derived([
    ...(parsedPage?.media?.images ?? []).map((img) => ({
      type: "image" as const,
      thumbnail: img.thumbnailUrl,
      original: img.originalUrl,
    })),
    ...(parsedPage?.media?.videos ?? []).map((video) => ({
      type: "video" as const,
      thumbnail: "",
  ...
  let selectedVariation = $derived(
    parsedPage?.variations?.find((v) => v.id === selectedVariationId) ??
